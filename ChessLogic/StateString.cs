...
namespace ChessLogic
{
    public class StateString
    {
        ...
        {
            int empty = 0; // consecutive empty squares
            ...
            if (empty > 0) // if there are empty positions at the end of the row
            {
                sb.Append(empty);
            }
        }
        ...
        private void AddPiecePlacement(Board board)
        {
            for (int r = 0; r < 8; r++)
            {
                if (r != 0) // Add a / before each row except the 1st row
                {
                    sb.Append('/');
                }
        ...
        {
            bool castleWKS = board.CastleRightsKS(Player.White);
            bool castleWQS = board.CastleRightsQS(Player.White);
            bool castleBKS = board.CastleRightsKS(Player.Black);
            bool castleBQS = board.CastleRightsQS(Player.Black);
